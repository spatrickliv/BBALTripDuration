...
# Full model including interaction with age
Load the model to check convergence and results
```{r results_Afull}

load("dhglm_Afull.rda")
...
Check converge and model mixing
```{r}
check_all_diagnostics(dh_Afull)
```

Get the results and plot a few figures
```{r}
results.Afull <- summary(dh_Afull, pars = params2, probs = c(0.025, 0.5, 0.975))$summary
rownames(results.Afull)[1:(ncol(Xm.Afull) + ncol(Xv.Afull))] <-
    c(paste0("b_", colnames(Xm.Afull)),
      paste0("g_", colnames(Xv.Afull))
      )
print(results.Afull, digits = 2)

stan_plot(dh_Afull, show_density = TRUE, pars=params2) + ggtitle("Posterior distributions full model")
stan_dens(dh_Afull, pars=params2) + ggtitle("Posterior distributions full model")
stan_trace(dh_Afull, pars = params) + ggtitle("Trace plot full model")

```

#  Model with non-significant interactions removed but other fixed effects kept in
...
check_all_diagnostics(dh_sig)

```

Get the results and plot a few figures
```{r}
results.sig <- summary(dh_sig, pars = params2, probs = c(0.025, 0.5, 0.975))$summary
